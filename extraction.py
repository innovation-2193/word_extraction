...
def process_docx(file_buffer):
    with zipfile.ZipFile(file_buffer, 'r') as docx_zip:
        ...
        # ===============================================
        def update_page_num(text_to_update, page_num):
            ...
            return re.sub(r"(page|หน้า|หน้าที่)\s*([๐-๙0-9]+)", repl, text_to_update, flags=re.IGNORECASE)

        # 4. ประกอบร่างข้อความ (Assemble) แทรกทั้งหัวและท้ายในทุกหน้า
        final_output = ""
        if not pages:
        ...
        for i, page_text in enumerate(pages, start=1):
            ...
            # เสก Header และ Footer ใหม่ตามหมายเลขหน้า
            page_header = update_page_num(master_header, i)
            page_footer = update_page_num(master_footer, i)
            
            # แปะ Header ไว้ด้านบนของหน้านี้
            if page_header.strip():
                final_output += f"--- [ส่วนหัว หน้าที่ {i}] ---\n{page_header.strip()}\n\n"
                
            # แปะเนื้อหาหลักของหน้านี้
            final_output += f"--- [เนื้อหาหลัก หน้าที่ {i}] ---\n{page_text.strip()}\n\n"
            
            # แปะ Footer ไว้ด้านล่างของหน้านี้
            if page_footer.strip():
                final_output += f"--- [ส่วนท้าย หน้าที่ {i}] ---\n{page_footer.strip()}\n\n\n"
...
✅ แยกแสดง `[ส่วนหัว หน้าที่ X]` และ `[ส่วนท้าย หน้าที่ X]` ประกบหัวและท้ายให้ใน **ทุกๆ หน้า**
✅ รันตัวเลขหน้าอัตโนมัติ (รองรับเลขไทย)
""")
